...
public class AndFilterBlock extends LogicFilterBlock {
  ...
  /**
   * add the unCorrelated condition to the where branch, but first of all, need to find the corresponding
   * select for this unCorrelated condition, since the uncorrelated filter shall be just added to
   * the select (topselect) directly, but not the transformed (with subQ in where leading to join) select.
   *
   * @param transformedSelect
   * @param condition
   */
