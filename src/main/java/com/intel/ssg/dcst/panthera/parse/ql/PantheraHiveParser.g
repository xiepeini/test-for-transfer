...
*/
parser grammar PantheraHiveParser;

options
{
tokenVocab=PantheraHiveLexer;
output=AST;
...
// Package headers
@header {
package com.intel.ssg.dcst.panthera.parse.ql;
...
@members {
  ...
  public String getErrorMessage(RecognitionException e, String[] tokenNames) {
    ...
    for (int i = 0; i < tokenNames.length; ++i) {
      xlateNames[i] = PantheraHiveParser.xlate(tokenNames[i]);
    }
...
createStreamStatement
@init { msgs.push("create stream statement"); }
@after { msgs.pop(); }
    : KW_CREATE (ext=KW_EXTERNAL)? KW_STREAM ifNotExists? name=tableName
      (  like=KW_LIKE likeName=tableName
         tableLocation?
         tablePropertiesPrefixed?
       | (LPAREN columnNameTypeList RPAREN)?
    ...
    -> ^(TOK_CREATESTREAM $name $ext? ifNotExists?
         ^(TOK_LIKETABLE $likeName?)
         columnNameTypeList?
         tableComment?
         tablePartition?
...
dropStreamStatement
@init { msgs.push("drop stream statement"); }
@after { msgs.pop(); }
    : KW_DROP KW_STREAM ifExists? tableName -> ^(TOK_DROPSTREAM tableName ifExists?)
    ;
