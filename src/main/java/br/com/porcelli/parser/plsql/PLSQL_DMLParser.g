...
//        }
    ...
    private boolean enablesOverClause(String originalFunctionName) {
        ...
        if (functionName.equals("avg") || functionName.equals("corr")
                ...
                || functionName.equals("histogram_numeric")
                || functionName.equals("ngrams")
                || functionName.equals("context_ngrams")) {
            return true;
        }
        return false;
    }
...
query_sort_clause
    ...
    |    expression (COMMA expression)*
        ...
        -> ^(sort_key expression_list? (EXPR expression)*)
    ;

query_distribute_clause
    :    distribute_key by_key
    (    (LEFT_PAREN (select_key|with_key)) => LEFT_PAREN subquery RIGHT_PAREN
    |    (LEFT_PAREN)=> expression_list
    |    expression (COMMA expression)*
    )
        -> ^(distribute_key expression_list? (EXPR expression)*)
    ;

query_cluster_clause
    :    cluster_key by_key
    (    (LEFT_PAREN (select_key|with_key)) => LEFT_PAREN subquery RIGHT_PAREN
    |    (LEFT_PAREN)=> expression_list
    |    expression (COMMA expression)*
    )
        -> ^(cluster_key expression_list? (EXPR expression)*)
    ;
...
standard_function
    ...
    |    (first_value_key^|last_value_key^) 
            LEFT_PAREN!
                concatenation_wrapper (ignore_key! nulls_key | COMMA! boolean_constant)? 
            RIGHT_PAREN! over_clause
    |    (first_value_key^|last_value_key^)
    ...
    |    trim_key^
            LEFT_PAREN!
                (((leading_key|trailing_key|both_key) | (quoted_string|concatenation_wrapper) | (leading_key|trailing_key|both_key) (quoted_string|concatenation_wrapper)) from_key)?
                concatenation_wrapper
            RIGHT_PAREN!
    |    xmlagg_key^
            LEFT_PAREN! 
    ...
    |    substring_key^
            LEFT_PAREN!
                concatenation_wrapper from_key! numeric (for_key! numeric)?
            RIGHT_PAREN! 
    ;
...
over_clause
    :    over_key^
        LEFT_PAREN!
            (query_partition_clause | query_sort_clause | query_distribute_clause | query_cluster_clause)?
